#' Streamlined Trial Data Simulations and Analysis Using Weighted Log-rank Test For Piecewise Exponential Distribution
#'
...
#' @fws   Weighting strategy in the following format as examples: fws can contain multiple weighting strategies.
...
#'        }
#' @param nphDesign An nphDesign object created from nphDesign() function or wlr.power.maxcombo() function.
...
#' H0 = wlr.sim.pwexp(nSim=5, N = 100, A = 21, w=1.5, r=1, lambda0=log(2)/11.7, lambda1=log(2)/11.7*0.745,
#' cuts=NULL, targetEvents = c(60, 80), 
#' sf = "LDOF", overall.alpha = 0.025, side = 1, alpha = NULL,
#' logrank="Y", fws=fws, nphDesign = NULL)
#' 
#' @export 
wlr.sim.pwexp = function(nSim=10000, N = 672, A = 21, w=1.5, r=1, lambda0=log(2)/11.7, lambda1=log(2)/11.7*0.745, 
    cuts=NULL, dropOff0=0, dropOff1=0, targetEvents = c(290, 397, 496), 
    sf = "LDOF", overall.alpha = 0.025, side = 1, alpha = NULL,
    logrank="Y", fws=list(fws1, fws2, fws3), 
    nphDesign = NULL) {
  ...
  for (i in 1:nSim) {
    ...
    for (m in 1:M){    
      ...
      wlri = wlr.inference(datasets=dati, alpha = alpha, side = side, f.ws=fws[[m]])$test.results
